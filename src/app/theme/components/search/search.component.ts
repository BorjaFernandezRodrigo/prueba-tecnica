...
export class SearchComponent implements OnInit, OnChanges {
  ...
  @Input() valuesInput: Array<any>;
  @Input() titulo: string;
  @Input() options: Options;

  @Input() keysSearch: { name?: string; prop: string }[];

  @ViewChild('keyInput', { static: false }) fieldInput: ElementRef<HTMLInputElement>;
    ...
    if (typeof value === 'object') {
      return this.keysSearch.filter((keySearch) => {
        return (
          ...
          (keySearch.name &&
            keySearch.name.toLowerCase().indexOf(value['name'].toLowerCase()) === 0)
        );
      });
  ...
  newParamSerach(paramSerach: string) {
    const paramsSearch = this.formatParamsSearch(paramSerach);
    const valueSearch = this.searchValues({ name: paramsSearch.name, value: paramsSearch.value });
      ...
      return (filtro) => {
        return filtro[prop]
          ? new RegExp(regexp, 'gi').test(
              filtro[prop].toLocaleString('es-ES', { minimumFractionDigits: 2 }).trim(),
            )
          : null;
      };
    }
